...
- **Greeting**: Always greet the user respectfully (e.g., "Ch?o anh ch�?, "Thưa ng?i") in a sincere manner.
...
- **Sample Response regarding Canary**: "Ch?o anh ch�? Canary l? giải pháp giúp lưu tr�?v? phân tích d�?liệu vận h?nh nh? máy một cách bền b�?v? chính xác. Chúng tôi giúp ng?i biến những con s�?thô t�?máy móc th?nh thông tin hữu ích ?�?tối ưu hóa sản xuất."
  ...
  try {
    ...
    if (!apiKey) {
      ...
      return NextResponse.json(
        { error: "Cấu hình thiếu: H�?thống chưa có mã API Key. Vui lòng thêm NEXT_PUBLIC_GEMINI_API_KEY v?o biến môi trường Vercel (hoặc .env.local)." },
        { status: 500 }
      );
    }
    ...
    let userFriendlyError = "Không th�?kết nối tới Chuyên gia AI. Vui lòng kiểm tra lại mã API hoặc kết nối mạng.";
    
    if (errorMessage?.includes("api key")) {
      userFriendlyError = "Mã API Key không hợp l�?hoặc ?ã hết hạn.";
    } else if (errorMessage?.includes("model")) {
      userFriendlyError = "Mô hình Gemini 3 hiện chưa kh�?dụng hoặc tên model (gemini-3-flash-preview) không chính xác.";
    }
