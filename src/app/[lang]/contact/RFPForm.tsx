...
export default function RFPForm({ lang }: RFPFormProps) {
  ...
  if (isSubmitted) {
    return (
      <motion.div 
        ...
        className="military-panel rounded-[3rem] p-16 text-center space-y-8"
      >
        ...
        <button 
          ...
          className="px-8 py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl text-sm font-bold transition-all border border-white/10"
        >
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-3">
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white placeholder:text-slate-600 focus:border-blue-500/50 outline-none transition-all font-light"
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <select
              required
              className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white appearance-none focus:border-blue-500/50 outline-none transition-all font-light"
              value={formData.industry}
              onChange={e => setFormData({...formData, industry: e.target.value})}
            >
              <option value="" disabled className="bg-[#020617]">{isEn ? "Select Industry" : "Chọn ngành nghề"}</option>
              {industries.map(ind => (
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            >
              <option value="" disabled className="bg-[#020617]">{isEn ? "Select Solution" : "Chọn giải pháp"}</option>
              {solutions.map(sol => (
                <option key={sol.value} value={sol.value} className="bg-[#020617]">{sol.label}</option>
              ))}
            </select>
            <ChevronDown className="absolute right-6 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 pointer-events-none" />
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            >
              <option value="" disabled className="bg-[#020617]">{isEn ? "Select Scale" : "Chọn quy mô"}</option>
              {scales.map(scale => (
                <option key={scale} value={scale} className="bg-[#020617]">{scale}</option>
              ))}
            </select>
            <ChevronDown className="absolute right-6 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 pointer-events-none" />
          </div>
      ...
      {/* Message */}
      <div className="space-y-3">
        <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">{isEn ? "Technical Requirements / Notes" : "Yêu cầu kỹ thuật chi tiết"}</label>
        <textarea
          rows={5}
          placeholder={isEn ? "Describe your security goals..." : "Mô tả mục tiêu bảo mật của dự án..."}
          className="w-full bg-white/5 border border-white/10 rounded-3xl px-6 py-4 text-white placeholder:text-slate-600 focus:border-blue-500/50 outline-none transition-all font-light resize-none"
      ...
      <div className="pt-4">
        ...
        <p className="mt-6 text-slate-500 text-[10px] uppercase tracking-widest font-black flex items-center gap-2">
          <ShieldCheck className="w-3 h-3 text-blue-500" /> {isEn ? "Secure encrypted data transmission" : "Truyền dữ liệu mã hóa an toàn"}
        </p>
      </div>
    </form>
  );
